...
export class TaskService {
   ...
   */
  async getTaskById(playerId: number, taskId: number): Promise<Task | null> {
  ...
  async completeTask(
    ...
    try {
      ...
      const updatedTask = await this.taskRepository.updateProgress(
        playerId,
        taskId,
        'completed'
      );
   ...
   */
  async skipTask(playerId: number, taskId: number): Promise<{
    success: boolean;
    message: string;
  }> {
    try {
   ...
   */
  async getAllTasksForMission(playerId: number, missionId: number): Promise<Task[]> {
   ...
   */
  async getTaskDetails(playerId: number, taskId: number): Promise<Task | null> {
   ...
   */
  async updateProgress(playerId: number, taskId: number, status: TaskStatus): Promise<Task> {
   ...
   */
  private async checkMissionCompletion(playerId: number, missionId: number): Promise<{
    isCompleted: boolean;
    progress: number;
    totalPoints: number;
    earnedPoints: number;
    ...
    try {
      ...
      const completedTaskPoints = tasks.reduce((sum, t) => 
        t.status === 'completed' ? sum + (t.points || 0) : sum, 0);
      ...
      if (allRequiredCompleted && earnedPoints >= requiredPoints) {
        await this.missionRepository.updateProgress(
          playerId,
          missionId,
          'completed',
          earnedPoints
