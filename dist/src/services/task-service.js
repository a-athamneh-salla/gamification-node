...
class TaskService {
     ...
     */
    async getTasksByStore(storeId, missionId, status, page = 1, limit = 10) {
     ...
     */
    async getTaskWithProgress(storeId, taskId) {
    ...
    async skipTask(storeId, taskId) {
        try {
            ...
            const mission = await this.missionRepository.findByIdForStore(storeId, task.missionId);
            if (mission) {
                // Recalculate mission progress
                const tasks = mission.tasks || [];
                // Count completed and skipped tasks
                const completedOrSkippedTasks = tasks.filter((t) => t.status === 'completed' || t.status === 'skipped' || t.isOptional).length;
     ...
     */
    async getTasksForMission(storeId, missionId) {
